    ...
    Set-PSReadLineKeyHandler -Key Enter -ScriptBlock {
        ...
        if (`$line -match '[\u4e00-\u9fff]') {
            ...
            if (`$json) {
                ...
                if (`$r.error) {
                    Write-Host ""
                    Write-Host "[脱口] `$(`$r.error)" -ForegroundColor Red
                } elseif (`$r.level -eq "read" -and `$r.output) {
                    Write-Host ""
                    Write-Host `$r.output -ForegroundColor Green
                } elseif (`$r.command) {
                    Write-Host ""
                    Write-Host "  待执行: `$(`$r.command)" -ForegroundColor Yellow
                    if (`$r.warning) {
                        Write-Host "  `$(`$r.warning)" -ForegroundColor Red
                    }
                    ...
                    if (`$confirm -eq "" -or `$confirm -eq "y" -or `$confirm -eq "yes") {
                        ...
                        `$execJson = Invoke-TuokouApi "/execute?cmd=`$cmdEnc"
                        if (`$execJson) {
                            `$execR = `$execJson | ConvertFrom-Json
                            if (`$execR.output) {
                                Write-Host `$execR.output -ForegroundColor Green
                            }
                        }
                    } else {
                ...
                Write-Host ""
                Write-Host "[脱口] 守护进程未就绪，尝试重新启动..." -ForegroundColor Yellow
                Start-TuokouDaemon
                `$q = [System.Uri]::EscapeDataString(`$line)
                `$json = Invoke-TuokouApi "/translate?q=`$q"
                if (-not `$json) {
                    Write-Host "[脱口] 启动失败，请手动执行: python `$tuokouHandler daemon" -ForegroundColor Red
                }
            }
